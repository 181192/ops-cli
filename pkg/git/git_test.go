	...
	Describe("ValidateURL", func() {
		...
		It("returns an error on HTTPS Git URL", func() {
			Expect(git.ValidateURL("https://github.com/eksctl-bot/my-gitops-repo.git")).
				To(MatchError("got a HTTP(S) Git URL, but currently only supports SSH Git URLs"))
		})
	...
	Describe("Options", func() {
		Describe("Validate", func() {
			...
			It("returns an error on HTTPS Git URL", func() {
				...
				}.Validate()).To(MatchError("got a HTTP(S) Git URL, but currently only supports SSH Git URLs"))
			})
		...
		Describe("ValidateURL", func() {
			...
			It("returns an error on HTTPS Git URL", func() {
				Expect(git.Options{
					URL: "https://github.com/eksctl-bot/my-gitops-repo.git",
				}.ValidateURL()).To(MatchError("got a HTTP(S) Git URL, but currently only supports SSH Git URLs"))
			})
